...
import { EnquiryModalProps, EnquiryModel, EnquiryStatus } from "./Models";

function Enquiry() {
  const [enquiries, setEnquiries] = useState<Array<EnquiryModel>>([]);
  const [modalIsOpen, setModalIsOpen] = useState<boolean>(false);
  const [currentEnquiry, setCurrentEnquiry] = useState<EnquiryModel>(
    {} as EnquiryModel
  );
  useEffect(() => {
    fetch(process.env.REACT_APP_API + "Enquiries")
      .then((response) => response.json())
      .then((data: Array<EnquiryModel>) => {
        setEnquiries(data);
      });
  }, []);
  ...
  const openEnquiryInModal = (enq: EnquiryModel) => {
          ...
          <tbody>
            {enquiries.map((enq) => (
              <tr>
                <td
                  key={enq.id}
                  onClick={() => openEnquiryInModal(enq)}
                  className="bg-light"
                >
                ...
                {enq.status === EnquiryStatus.Empty ? (
                  <td className="text-secondary text-uppercase fw-bold table-secondary">
                    <h4 className="d-flex justify-content-center mt-3">
                      {enq.status}
                    </h4>
                  </td>
                ) : enq.status === EnquiryStatus.Booked ||
                  enq.status === EnquiryStatus.Submitted ? (
...
const EnquiryModal = (props: EnquiryModalProps) => {
  const { enq, modalIsOpen, onClose } = props;

  if (!modalIsOpen) return null;
  return (
    <Modal key={enq.id} isOpen={modalIsOpen}>
      <Animated
            ...
            <div className="d-flex justify-content-between">
              ...
              {enq.status === EnquiryStatus.Empty ? (
                <div className="d-flex fst-italic">
                  ...
                  <h1 className="text-light fw-bold">{enq.status}</h1>
                </div>
              ) : enq.status === EnquiryStatus.Booked ||
                enq.status === EnquiryStatus.Submitted ? (
                <div className="d-flex fst-italic">
                  <h1 className="mr-1">Enquiry Status:</h1>
                ...
                <div className="d-flex fst-italic">
                  ...
                  <div className="bg-danger rounded">
                    <h1 className="text-light fw-bold m-1">{enq.status}</h1>
                  </div>
                </div>
              ) : (
                <div className="d-flex fst-italic">
